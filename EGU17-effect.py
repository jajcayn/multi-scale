...
for i, ndx in zip(range(len(ndxs)), ndxs):
    ...
    mean_amp_windows[i] = np.mean(prg.amplitude[ndx])
...
plt.ylim([0, 1.5])
if SEASON is None:
    plt.title("PRG station: full year \n sliding window: %d years, shift: %d year" % (WINDOW_LENGTH, 1), size = 28)
else:
    plt.title("PRG station: %s \n sliding window: %d years, shift: %d year" % (''.join([mons[s] for s in SEASON]), 
