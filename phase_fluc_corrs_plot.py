...
with open("../scale-nets/bins/NCEP-SAT-annual-phase-fluc-1000FTsurrs-from-indices.bin", "rb") as f:
...
for index in INDICES:
    ...
    fname = ("../scale-nets/NCEP-SAT-annual-phase-fluc-%scorrs-sig-from-indices.png" % index)
    net.quick_render(field_to_plot = result, tit = tit, fname = fname, symm = True, whole_world = True)

with open("../scale-nets/bins/NCEP-SAT-annual-phase-fluc-1000FTsurrs-add-from-indices.bin", "rb") as f:
    surr_res = cPickle.load(f)
...
tit = ("NCEP number of significant with p-value %.2f" % (P_VAL))
fname = ("../scale-nets/NCEP-SAT-annual-phase-fluc-number-of-sig-from-indices.png")
net.quick_render(field_to_plot = no_sigs, tit = tit, fname = fname, symm = False, whole_world = True)
