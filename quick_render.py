...
def render_geo_field(data, lats, lons, u = None, v = None, symm = False, 
    ...
    m.drawmapboundary()
    m.drawparallels(np.arange(lats[0], lats[-1]+1, 10), dashes = [1,3], labels = [1,0,0,1], color = (.2, .2, .2), 
                    fontsize = 8.5)
    m.drawmeridians(np.arange(lons[0], lons[-1]+1, 10), dashes = [1,3], labels = [1,0,0,1], color = (.2, .2, .2), 
                    fontsize = 8.5)
    plt.title(title)
