...
def lorenz_deriv((x, y, z), t0, forcing = 0, sigma = 10., beta = 8/3., rho = 28.0):
    """
    function containing Lorenz '63 model with preset paramters for Lorenz attractor
    suitable for calling with scipy.integrate.odeint
    """
    
    return [sigma * (y - x) + forcing, x * (rho - z) - y + forcing, x * y - beta * z]
...
beta = 8/3.
rho = 28
INITIAL_POINT = (0, 0, 19)
INITIAL_NEIGHBOURHOOD = 1
T = 250
N = 16346 + 1000

t = np.linspace(0, T, N)
X0 = INITIAL_POINT + INITIAL_NEIGHBOURHOOD * (2 * np.random.random(3) - 1)
X = integrate.odeint(lorenz_deriv, X0, t, args = (0, sigma, beta, rho))
...
for [idx1, idx2] in names:
        ...
        import cPickle
        with open("CMI-coh-%s-%s--Lorenz.bin" % (idx1, idx2), "wb") as f:
        ...
        plt.savefig("CMI%s-%s--Lorenz.png" % (idx1, idx2))
        plt.close()
        ...
        # plt.savefig("AAindex_vs_Oulu_cosmic-surrs_from_cosmic_data.png")
        # plt.savefig("AAindex_vs_%s_cosmic-surrs-from-cosmic-data.png" % aa.location[:-12])
        plt.savefig("coherence%s-%s--Lorenz.png" % (idx1, idx2))
