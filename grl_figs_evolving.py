...
def render(diffs, meanvars, stds = None, subtit = '', percentil = None, phase = None, fname = None):
    ...
    p1, = ax1.plot(diffs[0], color = '#E89335', linewidth = 2, figure = fig) #19BF86
    if percentil != None:
        for pos in np.where(percentil[:, 0] == True)[0]:
            ax1.plot(pos, diffs[0][pos], 'o', markersize = 8, color = '#E89335')
    ax1.axis([0, cnt-1, diff_ax[0], diff_ax[1]])
    ax1.set_xlabel('middle year of %.2f-year wide window' % (WINDOW_LENGTH / 365.25), size = 17)
    ax1.set_ylabel('difference in cond. means in temperature [$^{\circ}$C]', size = 17)
    ...
    tit += subtit
    plt.text(0.5, 1.05, tit, horizontalalignment = 'center', size = 20, transform = ax1.transAxes)
...
fn = ("debug/PRGlong1000MFevolvingSeasons.pdf")  
SEASON = None  
...
fig = plt.figure(figsize = (14,20), frameon = False)
gs = gridspec.GridSpec(2, 1)
gs.update(left = 0.05, right = 0.9, top = 0.9, bottom = 0.1, wspace = 0.25, hspace = 0.25)
...
for i in range(2):
    ...
    p1, = ax.plot(diffs[0], color = '#E89335', linewidth = 2, figure = fig)
    if percentil != None:
        for pos in np.where(percentil[:, 0] == True)[0]:
            ax.plot(pos, diffs[0][pos], 'o', markersize = 8, color = '#E89335')
    ax.axis([0, cnt-1, diff_ax[0], diff_ax[1]])
    ax.set_xlabel('middle year of %.2f-year wide window' % (WINDOW_LENGTH / 365.25), size = 17)
    ax.set_ylabel('difference in cond. means in temperature [$^{\circ}$C]', size = 17)
