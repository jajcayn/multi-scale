...
def render(diffs, meanvars, stds = None, subtit = '', percentil = None, phase = None, fname = None):
    ...
    ax1.set_ylabel('DIFFERENCE IN COND. MEAN SATA [$^{\circ}$C]', size = 24)
...
fn = ("debug/PRGlong1000ARevolvingSeasons.eps")  
# SEASON = None  
...
# np.savetxt('debug/PRGlong1000FTevolving.txt', to_txt, fmt = '%.3f')



for se in seas:
    SEASON = ''.join([mons[m-1] for m in se])
    with open('data_temp/PRGlong1000ARevolution%s.bin' % SEASON, 'rb') as f:
        data = cPickle.load(f)
        for k, v in data.iteritems():
            locals()[k + SEASON] = v
            print k
