...
def render(diffs, meanvars, stds = None, subtit = '', percentil = None, phase = None, fname = None):
    ...
    p2, = ax1.plot(diffs[1], color = '#BF3919', linewidth = 2.5, figure = fig)
...
fn = 'LONG--HAM1000FTevolution'
fn_out = 'LONG--HAM1000FTevolution'
...
for i in range(2):
    ...
    if stds is not None:
        ax.fill_between(np.arange(0,diffs[1].shape[0],1), diffs[1], stds[0],
                         facecolor = "#CA6D57", edgecolor = "#CA6D57", alpha = 0.5)
