import numpy as np
from src.data_class import load_station_data, load_AAgeomag_data, load_bin_data
...
for LEVEL in LEVELS:
    for GRID_POINT in GRID_POINTS:
        ...
        if STATION:
            ...
            temp = load_bin_data(fname, date(1950, 1, 1), date(2001, 1, 1), True)
        ...
        np.savetxt(fname[:-3] + "txt", result, fmt = '%.4f')


# ax = plt.subplot(211)
# plt.title("COHERENCE AAindex vs. %s SAT %.1fN x %.1fE -- %d - %d" % (LEVEL, GRID_POINT[0], GRID_POINT[1], y1, y2), size = 30)
